...
class Mental():
    ...
    def train(self):
        ...
        train_x_smote, train_y_smote = smote.fit_resample(self.train_x.reshape(-1, self.TIME_SLOT * self.feature_nums), self.train_y)
        train_x_smote = train_x_smote.reshape(-1, self.TIME_SLOT, self.feature_nums)
        train_x_smote = train_x_smote.reshape(train_x_smote.shape[0], -1)  # Flatten the input for FFN
        self.model_smote = self.create_model(input_shape=(self.TIME_SLOT * self.feature_nums,))
        self.model_smote.fit(train_x_smote, train_y_smote, epochs=1000, batch_size=self.BATCH_SIZE, validation_split=0.2)

    def test(self):
        # 오버샘플링 모델 예측
        test_x_flat = self.test_x.reshape(self.test_x.shape[0], -1)  # Flatten the input for FFN
        pred_smote = self.model_smote.predict(test_x_flat)
