...
class Mental():
    ...
    def make_ds(self, d):
        ...
        self.train_dataset = self.train_dataset.shuffle(self.train_size)

        # 오버샘플링 적용
        x_train = np.array(list(self.train_dataset.map(lambda x, y: x["input_0"])))
        y_train = np.array(list(self.train_dataset.map(lambda x, y: np.argmax(y, axis=1))))  # 레이블 원-핫 인코딩 해제
        
        smote = SMOTE()
        x_train_resampled, y_train_resampled = smote.fit_resample(x_train, y_train)
        
        y_train_resampled = to_categorical(y_train_resampled, num_classes=len(self.CLASS))  # 다시 원-핫 인코딩

        self.train_dataset = tf.data.Dataset.from_tensor_slices((x_train_resampled, y_train_resampled)).batch(self.BATCH_SIZE)
