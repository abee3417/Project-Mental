...
from keras.layers import LSTM, Input, MultiHeadAttention, Attention, Reshape, Dense, Activation, \
    LayerNormalization, Concatenate, Flatten, Dropout, Bidirectional, GRU, Add, Masking, Subtract
        ...
        for id in self.id_list:
            ...
            for d_idx, d_row in enumerate(self.D_LIST):
                ...
                if len(y_tmp) > 1:
                    ...
                    # if d_idx == 0:
                    #     if max_len_0 < len(y_tmp):
                    #         max_len_0 = len(y_tmp)
                    # elif d_idx == 1:
                    #     if max_len_1 < len(y_tmp):
                    #         max_len_1 = len(y_tmp)
                    # elif d_idx == 2:
                    #     if max_len_2 < len(y_tmp):
                    #         max_len_2 = len(y_tmp)
    ...
    def create_model(self):
        ...
        for i in range(2):
            ...
            if i == 0:
                ...
                pass
                # pass
                # input = Input(shape=(130, 1), name=input_name, batch_size=self.BATCH_SIZE)

            list_input.append(input)

            input = Masking(mask_value=-1)(input)
            lstm = Bidirectional(LSTM(1))(input)
            lstm = Reshape((1, lstm.shape[1]))(lstm)  # 1 x S
            # state = Reshape((1, state.shape[1]))(state)  # 1 x S
            # lstm_attn = Attention()([state, lstm])  # 1 X S = Q (1 x S), V (T X S)
            list_concat.append(lstm)

        z = Concatenate(axis=1)(list_concat)
        # z_attn = MultiHeadAttention(num_heads=2, key_dim=2)(z, z)
        # z = LayerNormalization(axis=-1)(z_attn)
        z = Flatten()(z)
        # z = Dense(3)(z)
        # z = Activation(activation=swish)(z)
