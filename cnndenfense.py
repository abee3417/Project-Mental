...
class Mental():
    ...
    def create_model(self):
        ...
        for i in range(2):
            ...
            conv = Conv1D(filters=32, kernel_size=3, padding='same', activation='relu')(input)
            conv = MaxPooling1D(pool_size=2)(conv)
            conv = Dropout(0.3)(conv)
            conv = Conv1D(filters=64, kernel_size=3, padding='same', activation='relu')(conv)
            conv = MaxPooling1D(pool_size=2)(conv)
            conv = Dropout(0.3)(conv)
            conv = Flatten()(conv)
        ...
        z = Dense(64, activation='relu')(z) #dense layer neural count
        z = Dropout(0.5)(z)  #drop out비율 
    ...
    def make_ds(self, d):
        ...
        self.train_size = int(data_size * 0.8) #testsize 1-datasize
    ...
    def train(self):
        ...
        early_stopping = keras.callbacks.EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True) #overfiting 방지
        try:
            self.model.fit(
