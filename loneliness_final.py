...
class Mental():
    def __init__(self):
        ...
        self.features = [
            "check1_value", "check3_value", "check4_value", "check5_value",
            "service1", "service3", "service8",
            "service9", "service10", # 여기까지 고정 feature
        ]
    ...
    def preprocessing(self):
        ...
        self.np_y = np.ones((len(self.id_list), 3), dtype=np.float32) * -1
        ...
        # 결측값 처리: -1로 표시된 결측값을 평균으로 대체
        for i in range(self.np_x.shape[2]):
            feature_col = self.np_x[:, :, i]
            feature_col[feature_col == -1] = np.nan
            mean_value = np.nanmean(feature_col)
            self.np_x[:, :, i] = np.nan_to_num(feature_col, nan=mean_value)

        # 표준화
        scaler = StandardScaler()
        self.np_x = scaler.fit_transform(self.np_x.reshape(-1, self.feature_nums)).reshape(self.np_x.shape)
    ...
    def train(self):
        ...
        train_x_smote_enn, train_y_smote_enn = smote_enn.fit_resample(self.train_x, self.train_y)

        self.model_smote_enn = self.create_model()
        ...
        grid_search = GridSearchCV(self.model_smote_enn, param_grid, cv=3, scoring='accuracy', n_jobs=-1, verbose=2)
        grid_search.fit(train_x_smote_enn, train_y_smote_enn)
        self.model_smote_enn = grid_search.best_estimator_

        print(f"Best parameters found: {grid_search.best_params_}")

    def test(self):
        # SMOTEENN 모델 예측
        pred_smote_enn = self.model_smote_enn.predict_proba(self.test_x)[:, 1]
        pred_binary = np.where(pred_smote_enn > 0.5, 1, 0)
